...
class CustomisedRoleTest extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "CustomisedRole.key" should "parse a key" in {
    ...
    val parseResult =
      fastparse.parse(sourceString, CustomisedRole.key(_)) match {
  ...
  "CustomisedRole.pair" should "parse a pair" in {
    ...
    val parseResult =
      fastparse.parse(sourceString, CustomisedRole.pair(_)) match {
        case Parsed.Success(value, _) => Right(value)
        case failure: Parsed.Failure  => Left(failure)
      }
  ...
  "CustomisedRole.parameters" should "parse a list of parameters" in {
    ...
    val parseResult =
      fastparse.parse(sourceString, CustomisedRole.parameters(_)) match {
        case Parsed.Success(value, _) => Right(value.toMap)
        case failure: Parsed.Failure  => Left(failure)
      }
    parseResult.value.shouldBe(
      Map(
        "ssh_keys_bucket" -> SingleParamValue("bucket"),
        "ssh_keys_prefix" -> SingleParamValue("Team")
      )
    )
  }

  "CustomisedRole.formInputTextToVariables" should "parse a map of variables" in {
    val sourceData = CustomisedRole.formInputTextToVariables(
      "ssh_keys_bucket: bucket, ssh_keys_prefix: Team"
    )
    println(s"\n\n\n\n\n\n sourceData = $sourceData ")
    println(s" sourceData.value = ${sourceData.value} \n\n\n\n\n\n")
    sourceData.value.shouldBe(
      Map(
        "ssh_keys_bucket" -> SingleParamValue("bucket"),
        "ssh_keys_prefix" -> SingleParamValue("Team")
      )
  ...
  "CustomisedRole.formInputTextToVariables" should "parse variable lists" in {
    CustomisedRole
      .formInputTextToVariables("packages: [python-pip, emacs24]")
      .value
  ...
  "ParamValue.format" should "round trip param values" in {
    ...
    roundTrip(
      DictParamValue(
        Map(
          "elasticsearch" -> SingleParamValue(
            "http://host.domain:port/flying/monkeys"
          )
        )
      )
    ) shouldBe DictParamValue(
      Map(
        "elasticsearch" -> SingleParamValue(
          "http://host.domain:port/flying/monkeys"
        )
  ...
  "CustomisedRole.formInputTextToVariables" should "parse variable dicts" in {
    val result = CustomisedRole.formInputTextToVariables(
      "repositories: {elasticsearch: 'http://host.domain:port/flying/monkeys'}"
    )
    result.value
      .shouldBe(
        Map(
          "repositories" -> DictParamValue(
            Map(
              "elasticsearch" -> SingleParamValue(
                "http://host.domain:port/flying/monkeys"
              )
