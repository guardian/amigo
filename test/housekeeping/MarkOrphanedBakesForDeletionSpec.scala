...
class MarkOrphanedBakesForDeletionSpec extends AnyFlatSpec with Matchers {
  ...
  val orphanBakes = List(
    fixtureBakeDbModel("recipe-x", None),
    fixtureBakeDbModel("recipe-y", None)
  )
  val otherBakes = List(
    fixtureBakeDbModel("recipe-1", None),
    fixtureBakeDbModel("recipe-2", None)
  )
  ...
  "findOrphanedBakeIds" should "return empty when there are no orphaned bakes" in {
    ...
    val markedBakes =
      MarkOrphanedBakesForDeletion.findOrphanedBakeIds(recipeIds, otherBakes)
  ...
  it should "only find the orphaned bakes" in {
    ...
    val markedBakes = MarkOrphanedBakesForDeletion.findOrphanedBakeIds(
      recipeIds,
  ...
  it should "find all the orphaned bakes" in {
    val recipeIds =
      Set(RecipeId("recipe-1"), RecipeId("recipe-3"), RecipeId("recipe-4"))
    val markedBakes = MarkOrphanedBakesForDeletion.findOrphanedBakeIds(
      recipeIds,
      otherBakes ++ orphanBakes
    )
