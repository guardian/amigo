...
class MarkOldUnusedBakesForDeletionSpec extends AnyFlatSpec with Matchers {
  ...
  val oldBake1: Bake = fixtureBake(
    fixtureRecipe("recipe-1", oldDate),
    Some(AmiId("ami-1")),
    oldDate
  )
  val oldBake2: Bake = fixtureBake(
    fixtureRecipe("recipe-2", oldDate),
    Some(AmiId("ami-2")),
    oldDate
  )
  val newBake1: Bake = fixtureBake(
    fixtureRecipe("recipe-3", newDate),
    Some(AmiId("ami-3")),
    newDate
  )
  val newBake2: Bake = fixtureBake(
    fixtureRecipe("recipe-4", newDate),
    Some(AmiId("ami-4")),
    newDate
  )

  def fixtureBaseImage(
      baseImageId: String,
