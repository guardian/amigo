...
class PrismSpec extends AnyFlatSpec with Matchers {
  ...
  val httpConfiguration = new HttpConfigurationProvider(
  ...
  def withPrismClient[T](block: Prism => T): T = {
    Server.withRouter() {
      ...
      case GET(p"/instances") =>
        controllerComponents.actionBuilder {
          Results.Ok.sendResource("prism/instances.json")
        }
      case GET(p"/launch-configurations") =>
        controllerComponents.actionBuilder {
  ...
  it should "fetch all AWS account numbers" in {
    withPrismClient { prism =>
      ...
      accounts should be(
        Seq(AWSAccount("foo", "1234"), AWSAccount("bar", "5678"))
      )
    }
  }
      ...
      instances should be(
        Seq(
          Instance(
            "i-123456",
            AmiId("ami-fa123456"),
            AWSAccount("MyAccount", "1234567890")
          ),
          Instance(
            "i-b123456",
            AmiId("ami-abcd4321"),
  ...
  it should "fetch all launch configurations" in {
    withPrismClient { prism =>
      val launchConfigurations =
        Await.result(prism.findAllLaunchConfigurations(), 10.seconds)
      launchConfigurations should be(
        Seq(
          LaunchConfiguration(
            "MyName",
            AmiId("ami-abcdefg1"),
