...
export const createAmigoDeveloperPolicy = (
	...
	const packerRoleArn = `arn:${partition}:iam::${account}:role/PackerUser-PackerRole-*`;
	...
	return new GuDeveloperPolicyExperimental(scope, 'AmigoDeveloperPolicy', {
		...
		statements: [
			...
			// Download service account key file for Google group checking.
			allow(
				['s3:GetObject'],
				[
			...
			// Discover package lists.
			allow(['s3:ListBucket'], [dataBucket.bucketArn], {
				StringLike: { 's3:prefix': 'packagelists/*' },
			}),
			// Download package lists.
			allow(['s3:GetObject'], [`${dataBucket.bucketArn}/packagelists/*`]),
			...
			allow(['iam:GetInstanceProfile'], [packerInstanceProfile.valueAsString]),
