...
case class Attempt[A] private (underlying: Future[Either[Failure, A]]) {
  ...
  def fold[B](failure: Failure => B, success: A => B)(implicit
      ec: ExecutionContext
  ): Future[B] = {
  ...
  def isSuccess(implicit ec: ExecutionContext): Future[Boolean] =
    fold(_ => false, _ => true)
  def isFailure(implicit ec: ExecutionContext): Future[Boolean] =
    fold(_ => true, _ => false)
}
...
object Attempt {
  ...
  def traverseWithFailures[A, B](as: List[A])(
      f: A => Attempt[B]
  )(implicit ec: ExecutionContext): Attempt[List[Either[Failure, B]]] = {
  ...
  /** Option doesn't implement TraversableOnce so this is a little helper to
    * invert it.
    */
  def sequenceOption[A](
      option: Option[Attempt[A]]
  )(implicit ec: ExecutionContext): Attempt[Option[A]] = {
  ...
  )(implicit ec: ExecutionContext): Attempt[List[Either[Failure, A]]] = {
  ...
  /** Run f and convert all non fatal exceptions to UnknownFailures. Best
    * practice is to use catchNonFatal instead but this can be used when you
    * don't know what kind of exceptions will be thrown or you have a blasé
    * attitude.
    */
