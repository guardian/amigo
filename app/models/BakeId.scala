...
object BakeId {
  ...
  def toFilename(bakeId: BakeId) =
    s"${bakeId.recipeId.value}--${bakeId.buildNumber}.txt"

  def toMetadata(bakeId: BakeId) =
    s"Recipe=${bakeId.recipeId.value},BuildNumber=${bakeId.buildNumber}"
  ...
  def fromString(s: String): Either[DynamoReadError, BakeId] = s match {
    case DynamoFormatRegex(recipeId, buildNumber) =>
      Right(BakeId(RecipeId(recipeId), buildNumber.toInt))
    case _ =>
      Left(
        TypeCoercionError(new IllegalArgumentException(s"Invalid bake ID: $s"))
      )
  }
