...
class AppLoader extends ApplicationLoader with Loggable {
  override def load(context: Context): Application = {
    ...
    val configAndIdentity = for {
      identity <- if (isDev) Success(DevIdentity("amigo"))
    ...
    configAndIdentity.fold(err => {
      log.error(s"Failed to start application due to $err")
      throw err
    }, configAndIdentity => {
      val (config, identity) = configAndIdentity
      val newContext = context.copy(initialConfiguration = Configuration(config).withFallback(context.initialConfiguration))
