...
class BakeScheduler(
    scheduler: Scheduler,
    scheduledBakeRunner: ScheduledBakeRunner
) extends Loggable {
  ...
  private def scheduleBake(
    ...
    log.info(
      s"Scheduled recipe [$recipeId] to bake with schedule [${bakeSchedule.quartzCronExpression}]"
    )
  }
  ...
  private def triggerKey(recipeId: RecipeId): TriggerKey = new TriggerKey(
    recipeId.value
  )
